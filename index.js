...
bot.once('ready', () => {
    ...
    setInterval( () => {
        ...
        for (guildID of bot.guilds.cache.keyArray()) {
            ...
            greetingChannel = greetingGuild.channels.cache.find(channel => channel.name === 'general');
    ...
    bot.user.setActivity( activity[randomNumber].name , { type: activity[randomNumber].type })
...
bot.on('message', async (message) => {
    ...
    .toLowerCase().includes('québécois'))) {
        if(message.author.id === bot.user.id) return;
        if ((message.content.toLowerCase().includes('pas') &&
